...
wrapper = CLIPWrapper("openai/clip-vit-base-patch32", ["a cat", "a dog"], device=torch.device("cuda"))
logits = wrapper(pixel_values)  # pixel_values: (B,3,H,W) 已由 CLIPProcessor 处理
"""
...
class CLIPWrapper(nn.Module):
    def __init__(self, model_name: str, labels: List[str], device: torch.device):
        ...
        # 注册为 buffer，随模型一起移动到 device
        self.register_buffer("text_embeds", text_embeds)

    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        """
        输入：pixel_values（B,3,H,W） - 已由 CLIPProcessor 处理好的 tensor（float）
